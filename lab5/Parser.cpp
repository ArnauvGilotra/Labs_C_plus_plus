...
Shape* parseRectangleCommand(stringstream& line) {
    string name;
    float xcent, ycent, w, h;
    line >> name >> xcent >> ycent >> w >> h;
    //--same as circle
    // Do a simple error check
    if (line.fail()) {
        cout << "Error: invalid input" << endl;
    ...
    //--same as circle
    // Create the shape object and return a pointer to it EDITED FOR RECTANGLE SHAPE
    Shape* myShape = (Shape*) new Rectangle(name, xcent, ycent, w, h);
...
Shape* parseTriangleCommand(stringstream& line) {
    ...
    // Create the shape object and return a pointer to it EDITED FOR TRIANGLE SHAPE
